...
def calculate_precision_recall(prediction, label, mode, batch_idx, epoch):
    ...
    plt.savefig('/home/zjin04/data/uav_regression/regression_process/precision_recall_curve/' + str(mode) + '/precision_recall_curve_epoch_' + str(epoch) + 'batch_' + str (batch_idx) + '.png')
    plt.close()
...
def draw_roc_curve(prediction, label, mode, epoch, batch_idx):
    ...
    plt.savefig('/home/zjin04/data/uav_regression/regression_process/roc_curve/' + str(mode) + '/roc_curve_epoch_' + str(epoch) + '_batch_' + str (batch_idx) + '.png')
    plt.close()
...
def visualize_sum_testing_result(prediction, label, batch_id, epoch):
    ...
    if not os.path.exists("/home/zjin04/data/uav_regression/testing_result/epoch_" + str(epoch)):
        os.mkdir("/home/zjin04/data/uav_regression/testing_result/epoch_" + str(epoch))
    if not os.path.exists("/home/zjin04/data/uav_regression/testing_result/epoch_" + str(epoch) + "/sum"):
        os.mkdir("/home/zjin04/data/uav_regression/testing_result/epoch_" + str(epoch) + "/sum")
    ...
    for idx, _ in enumerate(prediction):
        prediction_output = prediction[idx].view(-1,100,100).cpu().detach()
        prediction_output = torch.squeeze(prediction_output)
        #print (prediction_output.shape)
        label_output = label[idx].view(-1,100,100).cpu().detach()
